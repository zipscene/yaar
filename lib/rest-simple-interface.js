...
class RESTSimpleInterface extends APIInterfaceJSONBase {
	...
	register(options, ...middleware) {
		...
		this.router.post(options.method.replace(/\./g, '/'), bodyParser.json({ limit: '5mb' }), this._mainRESTMiddleware.bind(this));

		if (/\.get$/.test(options.method) && options.model) {
			// Get object
			// Register route to handle GET on object path (get object)
			this.router.get(getObjectRoutePath(), this._mainRESTMiddleware.bind(this));
		} else if (/\.put$/.test(options.method) && options.model) {
			// Replace object
			// Register route to handle PUT on object path
			this.router.put(getObjectRoutePath(), bodyParser.json({ limit: '5mb' }), this._mainRESTMiddleware.bind(this));
			// Register route to handle POST on object root (create new)
			this.router.post(getObjectRootRoutePath(), bodyParser.json({ limit: '5mb' }), this._mainRESTMiddleware.bind(this));
