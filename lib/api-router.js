...
class APIRouter extends APICallRegistrar {
	...
	register(options, ...middleware) {
		...
		if (options.schema) {
			...
			middleware.unshift(paramParser);
		}
		...
		if (options.manualResponse) options.manualResult = true;
	...
	/**
	 * Creates the api-info route that returns API method information.
	 *
	 * @method registerAPIInfoRoute
	 * @param {Object} [options]
	 *   @param {Object} [options.extraModels] - Extra models to include with the api-info.  Map from
	 *     model name to model schema.
	 */
	registerAPIInfoRoute(options = {}) {
		if (this.apiInfo) return;
		this.apiInfo = new APIInfo(options);
		for (let version in this.versionRouters) {
			this.versionRouters[version].register({
				method: 'api-info',
				description: 'Retrieve information about API methods and syntax'
			}, this.apiInfo.getRoute(this.versionRouters[version]));
		}
	}
