...
class RESTSimpleInterface extends APIInterfaceJSONBase {
	...
	register(options, ...middleware) {
		...
		this.router.post(options.method.replace(/\./g, '/'), bodyParser.json({ limit: '5mb' }), this._mainRESTMiddleware);

		if (/\.get$/.test(options.method) && options.model) {
			// Get object
			// Register route to handle GET on object path (get object)
			this.router.get(getObjectRoutePath(), this._mainRESTMiddleware);
		} else if (/\.put$/.test(options.method) && options.model) {
			// Replace object
			// Register route to handle PUT on object path
			this.router.put(getObjectRoutePath(), bodyParser.json({ limit: '5mb' }), this._mainRESTMiddleware);
			// Register route to handle POST on object root (create new)
			this.router.post(getObjectRootRoutePath(), bodyParser.json({ limit: '5mb' }), this._mainRESTMiddleware);
		} else if (/\.delete$/.test(options.method) && options.model) {
			// Delete object
			// Register route to handle DELETE on object path
			this.router.delete(getObjectRoutePath(), this._mainRESTMiddleware);
		} else if (/\.query$/.test(options.method) && options.model) {
			// Register route to handle GET on object root (list objects)
			this.router.get(getObjectRootRoutePath(), this._mainRESTMiddleware);
		}
	}
