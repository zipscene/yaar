...
function RestRouter(options) {
	...
	this.mainRouter.use(function(req, res, next) {
		...
		var key;
		var params = req._zsApiRouterParams = {};
...
RestRouter.prototype.registerAPICall = function() {
	...
	var regArgs = [
		...
		function(req, res, next) {
			...
			// Add route name parameters to the params object
			if(req.params && typeof req.params == 'object' && req._zsApiRouterParams) {
				for(var key in req.params) {
					req._zsApiRouterParams[key] = req.params[key];
				}
			}
			next();
		}
