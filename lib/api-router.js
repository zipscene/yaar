...
class APIRouter extends APICallRegistrar {
	...
	constructor(options = {}) {
		...
		this.mainRouter = express.Router({ caseSensitive: true, strict: false }); // eslint-disable-line new-cap
	 ...
	 */
	version(versionNum) {
		if (this.versionRouters[versionNum]) return this.versionRouters[versionNum];

		let versionRouter = new VersionRouter(versionNum);
		this.versionRouters[versionNum] = versionRouter;
		this.versionsRouter.use(`/v${versionNum}`, versionRouter.getExpressRouter());
		return versionRouter;
	}
