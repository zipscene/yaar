...
class APICallRegistrar {
	 ...
	 *   than Express middleware.  Each function has the signature: function(context) and returns a Promise.
	 ...
	 *   @param {Object} middleware.context - The only parameter to middleware functions is the "context"
	 *     object, which contains information about the current call.
	 *     @param {Object} middleware.context.params - A map from keys to values containing user-supplied
	 *       API call parameters.
	 *     @param {Request} middleware.context.req - The Express request object.
	 *     @param {Response} middleware.context.res - The Express response object.
	 *     @param {String} middleware.context.version - The API version used to access this call.
	 */
	...
	/**
	 * Registers middleware that executes before all API calls for this registrar.  If these resolve
	 * with a value, that value is set as the result of the API call.  If these reject, that becomes
	 * the API call error.  If any resolve or reject, further middleware is not executed, and the API
	 * call middleware is not executed.
	 *
	...
	/**
	 * Registers middleware that executes after API calls return a result or error.  If these
	 * resolve with a result, the value is ignored.  If these reject, the error is logged,
	 * but is ignored.
	 *
	 * @method registerPostMiddleware
	 * @param {Object} options - Same options as register() but without a name.
	 * @param {Function} ...middleware - Same format as in register() .  The `context` object has
